...
public class KkuSystem implements FileNames, ReserveTasks, OptionalReserveTasks {
		...
		while (true) {
			...
			case "1":
				try {
					...
					int age = sc.nextInt();
					if (age < 0) {
						throw new Exception("Invalid age! Please enter a non-negative integer for age.");
					}
					...
					System.out.println("Invalid date format! Please enter a date in the yyyy-mm-dd format.");
					sc.nextLine();
				} catch (Exception e) {
			...
			case "7":
				try {
					...
					people.addPerson(name, id, age);
				} catch (InputMismatchException e) {
					System.out.println("Invalid input! Please enter valid name, id, and age.");
					sc.nextLine();
				} catch (Exception e) {
					System.out.println(e.getMessage());
					sc.nextLine();
	...
	public List<ReserveBlock> findReservedBlockAt(int clock) {
		List<ReserveBlock> blocksAtClock = new ArrayList<>();

		for (ReserveBlock block : reservations) {
			if (block.getClock() == clock) {
				blocksAtClock.add(block);
			}
		}

		return blocksAtClock;
	}

	@Override
	public List<ReserveBlock> findReservedBlocksBy(Person p) {
		List<ReserveBlock> personBlocks = new ArrayList<>();
		for (ReserveBlock block : reservations) {
			if (block.getBy().getId().equals(p.getId())) {
				personBlocks.add(block);
			}
		}
	...
	public List<Integer> findHoursForReservedBlock(ReserveBlock rb, LocalDate date) {
		...
		for (ReserveBlock reservation : reservations) {
			if (reservation.getDate().equals(date) && reservation.equals(rb)) {
	...
	public List<ReserveBlock> findReservedBlockAt(LocalDate date) {
		List<ReserveBlock> blocks = new ArrayList<>();
		for (ReserveBlock reservation : reservations) {
			if (reservation.getDate().equals(date)) {
				blocks.add(reservation);
			}
		}
		return blocks;
	}
	...
	public List<ReserveBlock> findReservedBlockAt(LocalDate date, int clock) {
		List<ReserveBlock> blocks = new ArrayList<>();
		for (ReserveBlock reservation : reservations) {
			if (reservation.getDate().equals(date) && reservation.getClock() == clock) {
				blocks.add(reservation);
			}
		}
		return blocks;
